...
% temporal parameters
tf = 100; % final time
num_frames = 500; % number of movie frames
time = linspace(0,tf,num_frames); % time span
...
phi =@(n,m,x,y) sin(((n*pi)/L).*x).*cos(((m*pi)/H).*y); % \phi_{n,m}

% temporal part of solution 
lambda =@(n,m) (n*pi/L)^2 + (m*pi/H)^2;% eigenvalues
h =@(n,m,t) sin(sqrt(lambda(n,m))*C*t);
...
for t=time
    ...
    U = phi(1,0,X,Y); % initialize the solution
